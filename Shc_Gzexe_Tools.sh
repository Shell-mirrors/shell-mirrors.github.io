...
function Install_Shc(){
	...
	if [ ! -z "$CentOS" ];then
		yum update -y
		yum groupinstall -y "Development Toolsi"
	elif [ ! -z "$Ubuntu" ];then
		apt-get update -y
...
function File_Name(){
	if [ ! -e $FilePath/$FileName ];then
		echo "\033[31mERROR\033[0m: 未找到\'$FilePath/$FileName\'文件,请重新输入!
		\033[3;33m按回车键继续\033[0m"
		read
		Check_PATH
	else
...
function Run_Shc(){
	...
	CFLAGS=-static shc -r -T -f $ALLPATH
	mv ${ALLPATH}* /home/shc
	mv /home/gzexe/$FilwName $ALLPATH
